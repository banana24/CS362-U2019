...
int main() {
   ...
   gainCard(2, &state, 2, currentPlayer); //Have to add these 4 cards to fix my logic for error checking, it wouldn't work without it.
   gainCard(2, &state, 2, currentPlayer);
   gainCard(2, &state, 2, currentPlayer);
   gainCard(2, &state, 2, currentPlayer);
   for( currentPlayer = 0; currentPlayer < 2; currentPlayer++){
	 ...
	 hold_Return = ambassadorEffect(&state, currentPlayer, choice1, choice2, handPos);
	 printf("%d: ", choice2); 
